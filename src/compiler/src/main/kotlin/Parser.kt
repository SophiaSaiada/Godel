...
object Parser : ParserBase() {
    ...
    private fun parseStatements(firstToken: Token?, restOfTokens: Iterator<Token>): ParseTreeNodeResult {
        ...
        return if (firstToken in setOf(TokenType.SimpleName, TokenType.DecimalLiteral, TokenType.Apostrophes, TokenType.Hash, TokenType.OpenParenthesis) || firstToken in setOf(Keyword.If, Keyword.Else, Keyword.Return, Keyword.False, Keyword.True, Keyword.Val, Keyword.Class, Keyword.Fun)) {
    ...
    private fun parseStatement(firstToken: Token?, restOfTokens: Iterator<Token>): ParseTreeNodeResult {
        ...
        return if (firstToken in setOf(TokenType.SimpleName, TokenType.DecimalLiteral, TokenType.Apostrophes, TokenType.Hash, TokenType.OpenParenthesis) || firstToken in setOf(Keyword.If, Keyword.Else, Keyword.Return, Keyword.False, Keyword.True)) {
    ...
    private fun parseFunctionalOrNullableType(firstToken: Token?, restOfTokens: Iterator<Token>): ParseTreeNodeResult {
        val nodeType = InnerNodeType.FunctionalOrNullableType
        return if (firstToken in setOf(TokenType.RightArrow)) {
            val (child0, nextToken1) = parseToken(TokenType.RightArrow).invoke(firstToken, restOfTokens)
            val (child1, nextToken2) = parseSpaceStar(nextToken1, restOfTokens)
            val (child2, nextToken3) = parseType(nextToken2, restOfTokens)
            ParseTreeNodeResult(
                ParseTreeNode.Inner(listOf(child0, child1, child2), nodeType),
    ...
    private fun parseTypeStar(firstToken: Token?, restOfTokens: Iterator<Token>): ParseTreeNodeResult {
        val nodeType = InnerNodeType.TypeStar
        return if (firstToken in setOf(TokenType.SimpleName, TokenType.OpenParenthesis)) {
    ...
    private fun parseTypeStarRest(firstToken: Token?, restOfTokens: Iterator<Token>): ParseTreeNodeResult {
    ...
    private fun parseExpression(firstToken: Token?, restOfTokens: Iterator<Token>): ParseTreeNodeResult {
        ...
        } else if (firstToken in setOf(TokenType.SimpleName, TokenType.DecimalLiteral, TokenType.Apostrophes, TokenType.Hash, TokenType.OpenParenthesis) || firstToken in setOf(Keyword.Return, Keyword.False, Keyword.True)) {
    ...
    private fun parseSimpleExpression(firstToken: Token?, restOfTokens: Iterator<Token>): ParseTreeNodeResult {
        ...
        } else if (firstToken in setOf(TokenType.Hash)) {
            val (child0, nextToken1) = parseLambda(firstToken, restOfTokens)
            ParseTreeNodeResult(
                ParseTreeNode.Inner(listOf(child0), nodeType),
                nextToken1
            )
    ...
    private fun parseAnythingButApostrophes(firstToken: Token?, restOfTokens: Iterator<Token>): ParseTreeNodeResult {
        ...
        return if (firstToken in setOf(TokenType.WhiteSpace)) {
            ...
            val (child0, nextToken1) = parseToken(TokenType.Hash).invoke(firstToken, restOfTokens)
            ParseTreeNodeResult(
                ParseTreeNode.Inner(listOf(child0), nodeType),
                nextToken1
            )
        ...
        } else if (firstToken in setOf(TokenType.RightArrow)) {
            val (child0, nextToken1) = parseToken(TokenType.RightArrow).invoke(firstToken, restOfTokens)
    ...
    private fun parseElvisExpression(firstToken: Token?, restOfTokens: Iterator<Token>): ParseTreeNodeResult {
        ...
        return if (firstToken in setOf(TokenType.SimpleName, TokenType.DecimalLiteral, TokenType.Apostrophes, TokenType.Hash, TokenType.OpenParenthesis) || firstToken in setOf(Keyword.Return, Keyword.False, Keyword.True)) {
    ...
    private fun parseReturnExpression(firstToken: Token?, restOfTokens: Iterator<Token>): ParseTreeNodeResult {
        ...
        } else if (firstToken in setOf(TokenType.SimpleName, TokenType.DecimalLiteral, TokenType.Apostrophes, TokenType.Hash, TokenType.OpenParenthesis) || firstToken in setOf(Keyword.False, Keyword.True)) {
    ...
    private fun parseInfixExpression(firstToken: Token?, restOfTokens: Iterator<Token>): ParseTreeNodeResult {
        ...
        return if (firstToken in setOf(TokenType.SimpleName, TokenType.DecimalLiteral, TokenType.Apostrophes, TokenType.Hash, TokenType.OpenParenthesis) || firstToken in setOf(Keyword.False, Keyword.True)) {
    ...
    private fun parseSimpleOrParenthesizedExpression(firstToken: Token?, restOfTokens: Iterator<Token>): ParseTreeNodeResult {
        ...
        return if (firstToken in setOf(TokenType.SimpleName, TokenType.DecimalLiteral, TokenType.Apostrophes, TokenType.Hash) || firstToken in setOf(Keyword.False, Keyword.True)) {
    ...
    private fun parseStatementOrBlock(firstToken: Token?, restOfTokens: Iterator<Token>): ParseTreeNodeResult {
        ...
        } else if (firstToken in setOf(TokenType.SimpleName, TokenType.DecimalLiteral, TokenType.Apostrophes, TokenType.Hash, TokenType.OpenParenthesis) || firstToken in setOf(Keyword.If, Keyword.Else, Keyword.Return, Keyword.False, Keyword.True, Keyword.Val, Keyword.Class, Keyword.Fun)) {
            val (child0, nextToken1) = parseStatement(firstToken, restOfTokens)
            ParseTreeNodeResult(
                ParseTreeNode.Inner(listOf(child0), nodeType),
                nextToken1
            )
        } else throw CompilationError("not matching alternative for firstToken \"$firstToken\" in parseStatementOrBlock")
    }
    ...
    private fun parseLambda(firstToken: Token?, restOfTokens: Iterator<Token>): ParseTreeNodeResult {
        val nodeType = InnerNodeType.Lambda
        return if (firstToken in setOf(TokenType.Hash)) {
            val (child0, nextToken1) = parseToken(TokenType.Hash).invoke(firstToken, restOfTokens)
        ...
        } else throw CompilationError("not matching alternative for firstToken \"$firstToken\" in parseLambda")
    }

    private fun parseLambdaParametersStar(firstToken: Token?, restOfTokens: Iterator<Token>): ParseTreeNodeResult {
    ...
    private fun parseLambdaParametersRest(firstToken: Token?, restOfTokens: Iterator<Token>): ParseTreeNodeResult {
