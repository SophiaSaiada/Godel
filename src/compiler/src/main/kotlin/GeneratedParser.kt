...
fun parseStatements(firstToken: Token?, restOfTokens: Iterator<Token>): ParseTreeNodeResult? {
    ...
    return if (firstToken?.type in listOf(TokenType.Whitespace, TokenType.OpenParenthesis, TokenType.DecimalLiteral, TokenType.Apostrophes) || firstToken?.type == TokenType.Keyword && firstToken.content in listOf(Keyword.False.asString, Keyword.True.asString, Keyword.If.asString, Keyword.Val.asString)) {
        val (children, nextToken) =
...
fun parseStatementsRest(firstToken: Token?, restOfTokens: Iterator<Token>): ParseTreeNodeResult? {
    ...
    return if (firstToken?.type in listOf(TokenType.SEMI)) {
        val (children, nextToken) =
            composeParseCalls(parseToken(TokenType.SEMI), ::parseStatements).invoke(firstToken, restOfTokens)
        ParseTreeNodeResult(ParseTreeNode.Inner(children, nodeName), nextToken)
...
fun parseAnythingButApostrophes(firstToken: Token?, restOfTokens: Iterator<Token>): ParseTreeNodeResult? {
    ...
    return if (firstToken?.type in listOf(TokenType.SEMI)) {
        val (children, nextToken) =
            composeParseCalls(parseToken(TokenType.SEMI)).invoke(firstToken, restOfTokens)
        ParseTreeNodeResult(ParseTreeNode.Inner(children, nodeName), nextToken)
...
fun parseAnythingEndsWithApostrophes(firstToken: Token?, restOfTokens: Iterator<Token>): ParseTreeNodeResult? {
    ...
    return if (firstToken?.type in listOf(TokenType.SEMI, TokenType.Whitespace, TokenType.Dot, TokenType.Colon, TokenType.Assignment, TokenType.OpenParenthesis, TokenType.CloseParenthesis, TokenType.OpenBraces, TokenType.CloseBraces, TokenType.DecimalLiteral, TokenType.SimpleName, TokenType.Unknown) || firstToken?.type == TokenType.Keyword && firstToken.content in listOf(Keyword.False.asString, Keyword.True.asString, Keyword.Val.asString, Keyword.If.asString, Keyword.Else.asString)) {
        val (children, nextToken) =
...
fun parseSpacePlus(firstToken: Token?, restOfTokens: Iterator<Token>): ParseTreeNodeResult? {
    ...
    return if (firstToken?.type in listOf(TokenType.Whitespace)) {
        val (children, nextToken) =
            composeParseCalls(parseToken(TokenType.Whitespace), ::parseSpacePlusRest).invoke(firstToken, restOfTokens)
        ParseTreeNodeResult(ParseTreeNode.Inner(children, nodeName), nextToken)
    } else return null
}
